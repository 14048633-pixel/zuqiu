...
LEAGUE_IDS = {
 ...
 '波甲': 25, '中超': 52, '瑞超': 26, '丹超': 84, '英超': 1, '德国杯': 43,
 '比甲': 14, '葡超': 2, '芬超': 55, '意乙': 34, '阿甲': 85,
}
def deaccent(s):
...
print('出单场次:', len(st))
date_from = '2026-08-23T00:00:00Z'; date_to = '2026-08-24T23:59:59Z'
ev_index = collections.defaultdict(list)
lids = {LEAGUE_IDS.get(m['league']) for m in st if LEAGUE_IDS.get(m['league'])}
...
def match(ev, hk, ak):
    ...
    if not th or not ta: return False
    return ((th <= ak or ak <= th) and (ta <= hk or hk <= ta))
out = []
for m in sorted(st, key=lambda x: x['ct']):
    ...
    out.append((ct, m['league'], m['home'], m['away'], bb['name'], (hs, as_), '%s-%s' % (hs, as_), '%s vs %s' % (best.get('home_team'), best.get('away_team'))))
for o in sorted(out, key=lambda x: x[0]):
    print('[%s] %s %s vs %s | BEST=%s | 比分=%s | %s' % (o[0].strftime('%m-%d %H:%M'), o[1], o[2], o[3], o[4], o[6], o[7]))
