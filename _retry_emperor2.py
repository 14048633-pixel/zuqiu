...
for m in ms:
    ...
    for attempt in range(3):
        try:
            ...
            print("  retry %d %s vs %s: %r" % (attempt+1, m["target_home"], m["target_away"], e))
            time.sleep(6.0)
    if not ok:
        continue
    resp = od.get("response") or []
    if not resp:
        print("NO ODDS | %s vs %s" % (m["target_home"], m["target_away"]))
        time.sleep(4.0)
        continue
    eid = "apifb" + str(fid)
    ...
    print("OK | %s vs %s" % (m["target_home"], m["target_away"]))
    time.sleep(4.0)
