...
nol=[r for r in rows if r['_tag'] in ('终局无腿','LIVE无腿')]
print('===== 08-26 共 68 场 | 终局 %d | 进行中 %d | 无数据 %d =====' % (len(fin), len(live), len(nod)))
print()
...
print('===== 有腿终局 %d 场: 胜 %d / 负 %d / 走 %d =====' % (len(legfin), w, l, p))
...
print()
print('===== 终局无腿 %d 场(方向负EV/无方向, 仅比分参考) =====' % len(nol))
for r in sorted(nol, key=lambda x:x['ko_bjt'] or ''):
...
print()
print('===== 进行中 %d 场(有腿 %d) =====' % (len(live), sum(1 for r in live if r.get('_leg'))))
for r in sorted(live, key=lambda x:x['ko_bjt'] or ''):
    leg=r.get('_leg')
