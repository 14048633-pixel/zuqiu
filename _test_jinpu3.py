...
print('特征: away_gf=%s' % feats.get('away_gf'))
poi = rec['steps'].get('step8', {}).get('poisson', {})
print('λ: 主%s 客%s' % (poi.get('lambda_home'), poi.get('lambda_away')))
ua = rec.get('upset_assessment', {})
print('冷门引擎: 等级=%s 命中=%s' % (ua.get('level'), ua.get('triggered_count')))
print('  弱队优势:', ua.get('cold_items'))
s31 = rec['steps'].get('step31_ev_calc', {})
print('best_bet:', s31.get('best_bet'))
print()
print('实际: 金浦3-0 → best_bet客+0.5/1 输')
