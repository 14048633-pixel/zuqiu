# -*- coding: utf-8 -*-
import json, math, io, sys, os, re

ROOT = r"D:\足球分析"
SRC = os.path.join(ROOT, "analysis_records", "scan24h_analysis_20260822_2136.json")
OUT = os.path.join(ROOT, "analysis_records", "ligue1_full_20260822_2136.md")
...
ms = [m for m in data["matches"] if m["league"] == "法甲" and not m["home"].startswith("Olympique de Marseille")]
ms = sorted(ms, key=lambda m: m["ct"])
ages = [m["snap_age_h"] for m in ms if m["snap_age_h"] is not None]
age_str = "%.1f~%.1fh" % (min(ages), max(ages))

L = []
L.append("# ⚽ 法甲 %d 场全方向详细报告（2026-08-22 23:15 ~ 08-23 02:45）" % len(ms))
L.append("")
L.append("> 数据源：20:45 临场盘快照（距开赛 %s，新鲜）｜模型：泊松 λ + Dixon-Coles｜测试阶段仅记方向" % age_str)
L.append("")
...
for m in ms:
    ...
    L.append("- 快照年龄：%sh（阈值 12h → 新鲜✅）" % age)
