# -*- coding: utf-8 -*-
"""天皇杯 32 场 API-Football 盘口拉取 -> snapshots.csv"""
                ...
                for b in bk.get("bets") or []:
                    ...
                    if nm == "Match Winner":
                        for v in vals:
                            ...
                            out_rows.append([ts, eid, "天皇杯", ct, h, a, "apifb:"+bname, "h2h",
                                             (h if side=="home" else a if side=="away" else "Draw"), side, side, "", price, ts])
                        ...
                        for v in vals:
                            ...
                            out_rows.append([ts, eid, "天皇杯", ct, h, a, "apifb:"+bname, "totals",
                                             mm.group(2), ("over" if mm.group(1).lower()=="over" else "under"),
                                             mm.group(1).lower(), float(mm.group(2)), price, ts])
        print("OK | %s vs %s (rem=%s) rows=%d" % (m["target_home"], m["target_away"], rem, len([1 for r0 in resp for b in (r0.get('bookmakers') or [])])))
        time.sleep(1.8)
    print("总行:", len(out_rows))
    with open(SNAP, "a", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f)
        for row in out_rows:
            w.writerow(row)
    print("appended to snapshots.csv")
